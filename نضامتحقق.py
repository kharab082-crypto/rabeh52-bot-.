...
# =====================================================================
# 2. قاعدة البيانات (تم إضافة جدول التحقق)
# =====================================================================
class DB:
    def __init__(self):
    ...
    def is_user_verified(self, user_id):
        """ترجع True إذا كان المستخدم محققاً (أو أدمن)"""
    ...
    def is_user_blocked(self, user_id):
        """ترجع True إذا تم حظر المستخدم بسبب كثرة المحاولات الفاشلة"""
        if str(user_id) == str(ADMIN_ID):
            return False
    ...
    def create_campaign(self, owner_id, channel_link, target_members, price_per_member=5, points_per_join=2):
        ...
        try:
            ...
            if self.is_channel_forced(f"@{channel_username}"):
                return None, "هذه القناة موجودة بالفعل في قائمة الاشتراك الإجباري.", None
...
# =====================================================================
# 9. دوال التحقق (Captcha) - تُطلب فقط في أول بدء
# =====================================================================
FRUITS = ["تفاح", "موز", "برتقال", "عنب", "فراولة"]
CORRECT_FRUIT = "تفاح"  # يمكن تغييرها
...
def request_verification(chat_id):
    """إرسال رسالة التحقق للمستخدم (تستخدم فقط عند البدء)"""
    msg = (
        "🔐 **تحقق أمان**\n\n"
...
# =====================================================================
# 12. معالج الأوامر الرئيسي (مع التحقق عند البدء فقط)
# =====================================================================
...
# =====================================================================
# 13. معالج النصوص (تم إزالة استدعاءات التحقق المتكررة)
# =====================================================================
...
def handle_text(message):
    try:
        ...
        # ===== الأزرار العادية (لا تحتاج تحقق إضافي) =====
        if text == "📁 ملف دارك":
            points = db.get_points(user_id)
            if points < 1:
...
# =====================================================================
# 16. معالج الأزرار (معدل لإزالة التحقق المتكرر)
# =====================================================================
...
def handle_callback(call):
    try:
        ...
        # معالج التحقق (Captcha) - يتم استدعاؤه فقط عند البدء
        if data.startswith("captcha_"):
